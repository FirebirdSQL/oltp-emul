...
--create UNIQUE descending index wares_id_desc on wares(id); -- "unique" - ?! 08.02.2015, temply, performance investifgation of views v_min_xxx_yyy
    ...
    begin
        ...
        if ( a_new_optype = fn_oper_cancel_customer_order() ) then
            begin
                ...
                execute procedure sp_kill_qstorno_handle_qd4dd( :a_doc_id, :a_old_optype, fn_oper_cancel_customer_order() );
            end
...
create or alter view v_any_id_avl_res as
-- 08.02.2015: checking v`iew that at least one ware exists in qdistr for operation
-- 'creating customer reserve from avail. remainders', no matter of its ware_id
-- (otherwise get N thousands of NIR on wares when not yet any rows are visible for current Tx)
select x.ware_id
from qdistr x
where
    x.snd_optype_id = 2100
    and x.rcv_optype_id = 3300
...
create or alter view v_any_id_clo_ord as
-- 08.02.2015: checking v`iew that at least one ware exists in qdistr for operation
-- 'creating supplier order on the basis of client orders', no matter of its ware_id
-- (otherwise get N thousands of NIR on wares when not yet any rows are visible for current Tx)
select x.ware_id
from qdistr x
where
    x.snd_optype_id = 1000
    and x.rcv_optype_id = 1200
-- 3.0 only: supress building index bitmap on QDistr
order by ware_id, snd_optype_id, rcv_optype_id
rows 1
;
...
create or alter view v_any_id_ord_sup as
-- 08.02.2015: checking v`iew that at least one ware exists in qdistr for operation
-- 'Create invoice from supplier on the basis of OUR orders to him', no matter of its ware_id
-- (otherwise get N thousands of NIR on wares when not yet any rows are visible for current Tx)
select x.ware_id
from qdistr x
where
    x.snd_optype_id = 1200
    and x.rcv_optype_id = 2000
-- 3.0 only: supress building index bitmap on QDistr
order by ware_id, snd_optype_id, rcv_optype_id
rows 1
;
...
as
    ...
    declare c_make_amount_distr_2 cursor for (
        ...
        where
            ...
            -- 3.0: performance was enchanced in 3.0 for such special case,
            ...
            begin
                if ( v_storno_sub = 1 ) then -- ==>  distr_mode containing 'new_doc'
                    begin
                        ...
                        v_info = v_info || ', try UPD QDistr set id='||v_id;

                        -- execute procedure sp_add_perf_log(1, v_call, null, v_info); -- 10.02.2015, debug, temply
