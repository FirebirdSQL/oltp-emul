...
-- disabled here, see 'oltp_adjust_DDL.sql': index pstorned_worker_id on pstorned(worker_id);
commit;
...
recreate table mon_memory_consumption(
    ...
    ,rowset bigint -- for grouping records that related to the same measurement
    ,id dm_idb generated by default as identity constraint pk_mon_memory_consumption primary key
    ,dts timestamp default 'now'
...
recreate table mon_cache_memory (
    ...
    ,id dm_idb generated by default as identity constraint pk_mon_cache_memory primary key
    ,dts timestamp default 'now'
    ,elap_ms int -- duration of gathering mon$ info, milliseconds
);
...
begin
    if ( rdb$get_context('USER_SESSION', 'SEPARATE_WORKERS') = 0 ) then
        ...
        return -- rdb$get_context('USER_SESSION', 'WORKER_SEQUENTIAL_NUMBER');
            coalesce(
    ...
    begin
        ...
        begin
            ...
            insert into perf_log(
                ...
                ,'now'
                ,-1
                ,fn_get_stack( iif(:a_gdscode>=0, 1, 0) ) -- pass '1' to force write call_stack to perf_log if this is NOT expected test finish
                ,:a_char
...
as
    ...
    declare v_detailed_exc_text dm_info; -- prev: msg; 19.12.2018
    declare v_info dm_info;
        ...
        begin
            ...
            if ( v_storno_sub = 1 ) then
                ...
                values(
                    ...
                    :a_doc_list_id,
                    fn_this_worker_seq_no(), -- 12.08.2018
                    :v_ware_id,
                    :a_optype_id,
                    :v_rcv_optype_id,
                    :v_doc_data_id,
...
--        rows 1
--    ;
--select * from v_max_id_clo_res
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
--PLAN (V_MAX_ID_CLO_RES Q ORDER XQD_1000_3300_WA_SO_RO_WKR_SND)
--PLAN SORT (JOIN (V_MAX_ID_CLO_RES D NATURAL, V_MAX_ID_CLO_RES H INDEX (PK_DOC_LIST)))
--0 records fetched
--  19726 ms, 57397 read(s), 27270886 fetch(es)
--
...
as
begin
    -- NB! this SP is called from 'oltp_split_heavy_tabs_1.sql'
    for
        -- Extracts metadata of PSQL FUNCTIONS to be executed as statements in isql.
        -- Samples:
        -- select src from sys_get_func_ddl('', 0) -- output all funcs with EMPTY body (preparing to update)
        -- select src from sys_get_func_ddl('', 1) -- output all funcs with ORIGIN body (finalizing update)
        with
        s as(
