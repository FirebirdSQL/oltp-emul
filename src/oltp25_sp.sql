...
set autoddl off;
set list on;
select 'oltp25_sp.sql start at ' || current_timestamp as msg from rdb$database;
set list off;
commit;
...
begin
    ...
    -- 12.01.19: v_can_skip_order_clause must be ZERO in FB 3.0 ONLY
    -- and NOT for FB 2.5 (see comments for this in oltp30_sp.sql)
    v_can_skip_order_clause =
        decode( a_payment_oper,
...
create or alter procedure report_stat_per_tables as
begin
   ---- n/a in 2.5 ---
end
...
begin
    ...
    from (
        select
            u.stat_gr
           ,sum( iif( u.mon_system_flag = 1, m.mon$max_memory_used, 0) ) sys_memo_used
           ,sum( iif( u.mon_system_flag is distinct from 1, m.mon$max_memory_used, 0) ) usr_memo_used
        ...
        join
        (
            select 1 as stat_gr, a.mon$stat_id as stat_id, 0 as mon_system_flag, a.mon$attachment_id as att_id, a.mon$state as state
...
set list on;
set echo off;
select 'oltp25_sp.sql finish at ' || current_timestamp as msg from rdb$database;
set list off;
commit;
