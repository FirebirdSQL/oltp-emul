...
recreate table money_turnover_log(
    id dm_ids generated by default as identity constraint pk_money_turnover_log primary key using index pk_money_turnover_log
...
recreate table qdistr(
   id dm_ids
   constraint pk_qdistr primary key using index pk_qdistr -- FB 3.x: do NOT: "generated by default as identity", see s`p_make_qty_storno when C_MAKE_QTY_STORNO_MODE = 'UPD_ROW'
...
recreate table invnt_turnover_log(
   ...
   ,doc_data_id bigint
   ,optype_id bigint
   ,id dm_ids -- FB 3.x: do NOT `generated by default as identity`, we use bulk-getting new IDs (or trigger with gen_id) instead
   ,dts_edit timestamp default 'now' -- last modification timestamp
...
-- ÑÒÎÏ ÇÄÅÑÜ 01.10.2014, TODO: ÄÎÁÀÂÈÒÜ Â fn_halt_sign ÂÎÇÂÐÀÒ 1 ÄËß GDSCODE = 0 (SC 3.X TROUBLE!)
    ...
    begin
        if ( v_cnt < v_ctx_lim ) then
            begin
                ...
                rdb$set_context('USER_SESSION', 'PERF_LOG_'|| :v_cnt ||'_ID', c.id);           --  1
            ...
            begin
                ...
                for
                    select
                        ...
                        ,aux1, aux2
                    from tmp$perf_log g
                    --where unit = :a_unit and trn_id = :v_curr_tx
                    as cursor ct -- instead insert into perf_log +  delete from tmp$perf_log (single scan of tmp$perf_log using where current of)
                do begin
                    insert into perf_log(
                        id, id2, unit,exc_unit
                        ,fb_gdscode, trn_id, att_id, elapsed_ms
                        ...
                        ct.id, ct.id2, ct.unit, ct.exc_unit
...
when any do
    begin
        v_exc_info = iif( fn_is_lock_trouble(gdscode), 'lk_confl', v_exc_info);
        -- in a`utonomous tx:
...
begin
    ...
    begin
        ...
        if ( a_inserting_table = 'qdistr' ) then
            insert into zqdistr( -- 4debug: to analyze after test finish
                id,
                doc_id,
                ware_id,
                snd_optype_id,
            ...
            select
                ...
                :v_curr_tx
            from qdistr q
            where q.id = :a_inserting_id;
        else if ( a_inserting_table = 'qstorned' ) then
            insert into zqstorned( -- 4debug: to analyze after test finish
                id,
                doc_id,
                ware_id,
                snd_optype_id,
...
begin
    ...
    begin
        ...
        begin
            ...
            -- (FINAL point of ware turnover ==> remove data from qdistr into qstorned)
...
begin
    return fn_get_random_id('v_all_customers');
end
^
create or alter function fn_get_random_supplier returns bigint as
begin
     return fn_get_random_id('v_all_suppliers');
end
