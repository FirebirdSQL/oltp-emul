...
begin
    ...
    -- ||||||||||||||||||||||||||||||||||||||||||||||||||||||
    -- Replace source code of unit 'FN_GET-RANDOM_ID': inject inside it explicit names of all known views that
    -- are used in dynamic (and slow) SQL.
--    if (1 = 0) then
--    begin
--        for select src from tmp_remove_dyn_in_random_id into " " do suspend;
--    end
    -- ||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
-- #############################################################################
-- End of script oltp_split_heavy_tabs_1.sql; next to be run:  oltp_data_filling.sql
